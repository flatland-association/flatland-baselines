...
class FlatlandMetricsCallback(RLlibCallback):
    """
    Add `normalized_reward`, `total_reward` and `percentage_complete` evaluation metrics.
    """
    ...
    def on_episode_end(
        ...
        **kwargs,
        ...
        total_reward = sum([sum(agent_rewards) for agent_rewards in episode_rewards.values()])
        ...
        for state in TrainState:
            metrics_logger.log_value(
                f"num_{state.name}",
                sum([1 for agent in rail_env.agents if agent.state == state]),
            )
        ...
        metrics_logger.log_value(
            "elapsed_steps_ratio",
            rail_env._elapsed_steps / rail_env._max_episode_steps,
        )

        metrics_logger.log_value(
            "num_malfunctions",
            sum([agent.malfunction_handler.num_malfunctions for agent in rail_env.agents])
        )
