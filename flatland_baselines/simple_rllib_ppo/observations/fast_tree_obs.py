...
class FastTreeObs(ObservationBuilder):
    ...
    def find_all_cell_where_agent_can_choose(self):
        ...
        for h in range(self.env.height):
            for w in range(self.env.width):
                pos = (h, w)
                for dir in range(4):
                    possible_transitions = self.env.rail.get_transitions(((h, w), dir))
                ...
                for dir in range(4):
                    ...
                    for d in range(4):
                        if possible_transitions[d] == 1:
                            ...
                            if (
                                    new_cell in switches.keys()
                                    and pos not in switches.keys()
                            ):
    ...
    def required_agent_decision(self):
        ...
        for a in range(self.env.get_num_agents()):
            (
                ret_agents_on_switch,
                ret_agents_near_to_switch,
                ret_agents_near_to_switch_all,
                ret_agents_on_switch_all,
            ) = self.check_agent_decision(
                self.env.agents[a].position, self.env.agents[a].direction
            )
            ...
            agents_near_to_switch.update(
                {a: (ret_agents_near_to_switch and not ready_to_depart)}
            )

            agents_can_choose.update(
                {a: agents_on_switch[a] or agents_near_to_switch[a]}
            )

            agents_near_to_switch_all.update(
                {a: (ret_agents_near_to_switch_all and not ready_to_depart)}
            )
    ...
    def get(self, handle):
        ...
        # observation[4]  : int(agent.status == TrainState.READY_TO_DEPART)
        # observation[5]  : int(agent.status == TrainState.ACTIVE)
        # observation[6]  : int(agent.status == TrainState.DONE or agent.status == TrainState.DONE_REMOVED)
        ...
        if agent.state == TrainState.READY_TO_DEPART:
            ...
            observation[4] = 1
                ...
                or agent.state == TrainState.STOPPED
                or agent.state == TrainState.MALFUNCTION
        ):
            ...
            for dir_loop, branch_direction in enumerate(
                ...
                if possible_transitions[branch_direction]:
                    ...
                    new_cell_dist = distance_map[
                        handle, new_position[0], new_position[1], branch_direction
                    ]
                    if not (
                            np.isinf(new_cell_dist)
                            and np.isinf(current_cell_dist)
                    ):
        ...
        for h in self.env.get_agent_handles():
            ...
            if (
                    ...
                    or a.state == TrainState.STOPPED
                    or a.state == TrainState.MALFUNCTION
            ):
                nb_active_agents += 1
...
class FastTreeObservationBuilderGym(GymObservationBuilderWrapper):
    """
    Return the shortest distance to target for each
    """

    def __init__(self, max_depth=3):
        obs = FastTreeObs(max_depth=max_depth)
        dim = obs.observation_dim
        # obs = FlatlandFastTreeObservation(),
        # dim = flatland_fast_tree_observation.OBSERVATION_DIM
        super().__init__(obs,
                         gym.spaces.Box(low=-np.inf, high=np.inf, shape=(dim,), dtype=float))
