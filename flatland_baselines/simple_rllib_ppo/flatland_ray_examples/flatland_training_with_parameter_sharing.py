...
def train_with_parameter_sharing(
    ...
    args: Optional[argparse.Namespace] = None,  # args for add_rllib_example_script_args
    ...
    if isinstance(res, ResultDict):
        print(res)
    elif res.num_errors > 0:
...
def _get_algo_config_parameter_sharing(
    ...
    # TODO already done in run experiment!	INFO worker.py:1839 -- Calling ray.init() again after it has already been called.
    ray.init(
        **init_args,
    )
    ...
    env_name = f"flatland_env_{obs_builder_class().__class__.__name__}_{module_class.__name__ if module_class is not None else None}"
    print(env_name)
