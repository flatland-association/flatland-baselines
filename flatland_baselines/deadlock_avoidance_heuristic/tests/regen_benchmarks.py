...
if __name__ == '__main__':
    # copy from flatland-scenarios checkout, see analyse_results_simple.py for the same convention
    base_path = Path("../../../../flatland-scenarios/trajectories/malfunction_deadlock_avoidance_heuristics/")
    ...
    for data_dir, ep_id in l:
        trajectory = regen_trajectories(
            data_dir=Path("../../../../flatland-scenarios/trajectories/" + data_dir).resolve(),
            ep_id=ep_id,
            output_dir=Path(
                f"../../../../flatland-scenarios/trajectories/malfunction_deadlock_avoidance_heuristics/{ep_id.replace('_Level', '/Level')}")
        )
